...
router.post('/auth', bodyParser(), function *(){
  for (var i = 0; i < users.length; i++){
    if (this.request.body.username == users[i].username && this.request.body.password == users[i].password){
      this.session.user = users[i].username;
      this.redirect("/");
      return;
    }
  }
